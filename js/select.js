;(function($,window,document,undefined){$.fn.extend({select:function(options){var defaults={width:"180px",listMaxHeight:"200px",themeColor:"#00bb9c",fontColor:"#000",fontFamily:"'Helvetica Neue', arial, sans-serif",fontSize:"15px",showSearch:false,rowColor:"#fff",rowHoverColor:"#0faf03",fontHoverColor:"#fff",mainContent:"请选择",searchContent:"关键词搜索"}
var opts=$.extend(defaults,options);var $this=this;$this.hide();$this.after('\
                    ...
                    </div>');var $This=$this.next();$this.find("option").each(function(index,element){var $li=$('<li val='+$(element).val()+'>'+$(element).text()+'</li>');if($(element).prop("selected")){$li.addClass("list_current");$This.find(".select_content").text($(element).text());}
$This.find(".no_result").before($li);});if(opts.width.trim()!=""&&opts.width.trim()!="180px")
$This.css("width",opts.width.trim());if(opts.listMaxHeight.trim()!=""&&opts.listMaxHeight.trim()!="200px")
$This.find(".select_list_body").css("max-height",opts.listMaxHeight.trim());if(typeof(opts.showSearch)=="boolean"&&!opts.showSearch)
$This.find(".select_list_search").hide();if(opts.fontColor.trim()!=""&&opts.fontColor.trim()!="15px"){$This.find(".select_content,.select_input,li").css("color",opts.fontColor.trim());$This.find("li").hover(function(){$(this).css({"color":opts.fontHoverColor.trim()});},function(){$(this).css({"color":opts.fontColor.trim()});});}
if(opts.fontFamily.trim()!=""&&opts.fontFamily.trim()!="'Helvetica Neue', arial, sans-serif")
$This.find(".select_content,.select_input,li").css("font-family",opts.fontFamily.trim());if(opts.rowHoverColor.trim()!=""&&opts.rowHoverColor.trim()!="#0faf03"){$This.find("li").hover(function(){$(this).css({"background-color":opts.rowHoverColor.trim()});},function(){$(this).css({"background-color":opts.rowColor.trim()});});}
var list_height=$This.find(".select_list").height();function Search(self){var isAllHide=true;$This.find(".select_list_ul li").not(".no_result").each(function(index,element){if($(element).text().indexOf($(self).val())==-1)
$(element).hide();else if($(element).hasClass("list_current"))
$(element).hide();else{isAllHide=false;$(element).show();}});if(isAllHide)
$This.find(".select_list_ul .no_result").show();else
$This.find(".select_list_ul .no_result").hide();$This.find(".select_list").css("height","auto");list_height=$This.find(".select_list").height();}
$This.find(".select_input").css({"width":$This.find(".select_main").width()-41});$This.find(".select_arrow,.select_content,.select_arrow_after").click(function(){$This.find(".select_arrow,.select_arrow_after").toggleClass("cast_rotate");if($This.find(".select_arrow_after").hasClass("cast_rotate"))
$This.find(".select_arrow_after").css("margin-top","1px");else
$This.find(".select_arrow_after").css("margin-top","-1px");if($This.find(".select_list").hasClass("list_open")){$This.find(".select_list").removeClass("list_open").animate({"height":"0px"},200,function(){$This.find(".select_list").hide();});}
else
$This.find(".select_list").addClass("list_open").css({"height":"0px"}).show().animate({"height":list_height+"px"},200);});$This.find(".select_content").css({"width":$This.width()-$This.find(".select_arrow").width()-40+"px"});$This.find(".select_list_body").delegate("li","click",function(){if($(this).hasClass("no_result"))
return;$This.find(".select_list_body li").removeClass("list_current").show();$(this).addClass("list_current").hide();Search($This.find(".select_input"));$This.find(".select_content").text($(this).text());$This.attr({"val":$(this).attr("val")==null?'':$(this).attr("val"),"text":$(this).text()});$this.val($(this).attr("val"));list_height=$This.find(".select_list").height();$This.find(".select_content").trigger("click");});$This.find(".select_input").on("keyup",function(){Search(this);});$This.find(".select_list_body").mCustomScrollbar({theme:"minimal",advanced:{autoExpandHorizontalScroll:true}});$This.find(".select_list_body .mCSB_dragger_bar").css({"background-color":"#00bb9c"});if(opts.themeColor.trim()!=""&&opts.themeColor.trim()!="#00bb9c"){$This.find(".select_main,.select_input,.select_list").css("border-color",opts.themeColor.trim());$This.find(".select_list_body .mCSB_dragger_bar").css({"background-color":opts.themeColor.trim()});$This.find(".select_arrow").css("border-top-color",opts.themeColor.trim());}
